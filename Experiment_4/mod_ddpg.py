...
from mod_online_algorithm import OnlineAlgorithm
...
class DDPG(OnlineAlgorithm):
    ...
    def _init_tensorflow_ops(self):
        ...
        tf.scalar_summary("Rewards",tf.reduce_mean(self.rewards_placeholder))
        tf.scalar_summary("Terminals",tf.reduce_mean(self.terminals_placeholder))
        tf.scalar_summary("Mean Q Value", tf.reduce_mean(self.qf.output))
        tf.scalar_summary("Max Q Values",tf.reduce_max(self.qf.output))
        #merge summary and create a tensorflow op
        self.merged_summary_op = tf.merge_all_summaries()
        self.sess.run(tf.initialize_all_variables())
        #initialize a summary writer object
        self.summary_writer = tf.train.SummaryWriter(self.summary_dir,self.sess.graph)
