...
class OnlineAlgorithm(RLAlgorithm):
    ...
    def __init__(
        ...
        self.terminals_placeholder = tf.placeholder(tf.float32,
                                                    shape=[None, 1],
                                                    name='terminals')
        data_dir ="/home/shay93/Motor-Represenations/Data/Experiment_5/"
        if use_preinitialized_pool:
            with open(data_dir + "preinitialized_pool.npy","rb") as f:
        ...
        self.sess = tf.get_default_session() or tf.Session(config = config_tf)
                ...
                for _ in range(self.epoch_length):
                    ...
                    if self.pool.size >= self.min_pool_size:
                        for _ in range(self.n_updates_per_time_step):
                            self._do_training()
                        if itr % 1000 == 0:
                           self.summary_writer.add_summary(self.merged_summary,itr)
                    itr += 1
    ...
    def _do_training(self):
        ...
        if isinstance(ops[0], list):
            ...
            self.merged_summary = self.sess.run(self.merged_summary_op,feed_dict=feed_dict)
        else:
