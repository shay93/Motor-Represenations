...
class Shape_Autoencoder:
	...
	def build_graph(self):
		...
		h_fc2_reshape = tf.reshape(h_fc2,shape = [BATCH_SIZE,IMG_WIDTH // 4,IMG_WIDTH // 4,1])
		#now initialize some Weights for teh convolutional layer
		W_conv1 = tf.Variable(tf.truncated_normal(shape = [2, 2, 1, CONV_KERNELS_1], stddev = 0.1))
		#initialize a bias variable for the convolutional layer 
		b_conv1 = tf.Variable(tf.constant(0.1,shape = [CONV_KERNELS_1]))
		conv1 = tf.nn.conv2d(h_fc2_reshape,W_conv1,strides = [1,1,1,1], padding = 'SAME')
		...
		W_conv2 = tf.Variable(tf.truncated_normal(shape = [2,2,CONV_KERNELS_1,CONV_KERNELS_2] , stddev = 0.1))
		...
		pool2_flat = tf.reshape(pool2, shape = [IMG_WIDTH,-1])
		#initialize weights for last fully connected layer
		W_fc3 = tf.Variable(tf.truncated_normal(shape = [BATCH_SIZE*IMG_WIDTH*IMG_WIDTH*CONV_KERNELS_2 //(IMG_WIDTH*16),IMG_WIDTH * BATCH_SIZE],stddev = 0.1))
		b_fc3 = tf.Variable(tf.constant(0.1,shape = [IMG_WIDTH, IMG_WIDTH*BATCH_SIZE]))
		
		self.op_dict['y'] = tf.reshape(tf.sigmoid(tf.matmul(pool2_flat,W_fc3) + b_fc3),shape = [BATCH_SIZE,IMG_WIDTH,IMG_WIDTH,1])
  ...
  #     	self.op_dict['batch']*BATCH_SIZE,  # Current index into the dataset.
