...
def inverse_kinematics_2DOF(end_effector,link_length):
    ...
    theta_2 = np.arccos((end_effector_x**2 + \
    end_effector_y**2 - 2*(link_length**2))/(2*(link_length**2)))
    ...
    assert (np.all(forward_kinematics(states_2DOF,link_length) == end_effector)),\
...
def forward_kinematics(joint_angle_state,link_length):
	...
	n_dof = joint_angle_state[-1]
	#initialize the xpos and ypos
	xpos = np.ndarray(shape = joint_angle_state.shape[:-1] + (1,))
	ypos = np.ndarray(shape = joint_angle_state.shape[:-1] + (1,))
	for i in range(1,n_dof+1):
		xpos += link_length*np.cos(np.sum(joint_angle_state[...,:i]))
		ypos += link_length*np.sin(np.sum(joint_angle_state[...,:i]))
