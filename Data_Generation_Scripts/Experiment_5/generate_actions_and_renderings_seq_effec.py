...
def forward_kinematics(joint_angle_state,link_length):
    ...
    for i in range(1,n_dof+1):
        xpos += link_length*np.cos(\
                    np.expand_dims(np.sum(joint_angle_state[...,:i],axis = -1),-1))
        ypos += link_length*np.sin(\
                np.expand_dims(np.sum(joint_angle_state[...,:i],axis=-1),-1))
...
def random_3DOF_action_states(num_sequences = NUM_SEQUENCES,
    ...
    return actions_3DOF,init_states_3DOF,states_2DOF,next_states_2DOF

actions_3DOF,init_states_3DOF,states_2DOF,next_states_2DOF = random_3DOF_action_states()
rendered_arm_obs = Render_2DOF_arm(next_states_2DOF,\
                                            link_length_2DOF)

#get the end effector positions that the 3DOF arm should track
end_effector_3DOF = forward_kinematics(next_states_2DOF,\
                                       link_length_2DOF)

#now create the directory to save these images
with open(data_dir + "/" + "rendered_arm_obs_seq_effec.npy","wb") as f:
    #shape is [num_sequences,64,64,seq_length,1]
    pickle.dump(rendered_arm_obs,f)

with open(data_dir + "/" + "effector_3DOF_seq_effec.npy","wb") as f:
    #shape is [num_sequences,seq_length,2]
    pickle.dump(end_effector_3DOF,f)

with open(data_dir + "/" + "init_state_3DOF_seq_effec.npy","wb") as f:
    pickle.dump(init_states_3DOF,f)
