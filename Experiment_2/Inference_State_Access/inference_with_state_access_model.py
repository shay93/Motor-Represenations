...
class Inference_with_state_access(tensorflow_graph):
    ...
    def add_placeholder_ops(self):
        ...
        self.op_dict['y_'] = tf.placeholder(tf.float32,shape = [None,6,self.seq_max_length])
        return self.op_dict
    ...
    def add_model_ops(self):
        ...
        #first initialize fc layer variables for state inference
        W_fc_states = tf.Variable(tf.truncated_normal(shape = [self.hidden_units,3],stddev = 0.1),name="state/weights")
        b_fc_states = tf.Variable(tf.constant(0.,shape = [3]),name="state/bias")
        #initialize fc layer variables for action inference
        ...
        state_tensor = tf.pack(state_list,axis=2)
        action_tensor = tf.pack(action_list,axis=2)
        #each tensor of shape [None,3,self.seq_max_length]
        #now concatenate the tensors along their first dimension 
        self.op_dict["y"] = tf.concat(1,[state_tensor,action_tensor])
        #shape is [None,6,self.seq_max_length]
